           ...
           ...) 
{
    ...
    if (any(comp > min(object$ncomp))) 
      stop("the elements of 'comp' must be smaller or equal than ", min(object$ncomp), ".")
    ...
    bisect = object$variates$X[, comp] + object$variates$Y[, comp]
    cord.X = cor(A$X, bisect, use = "pairwise")
    cord.Y = cor(A$Y, bisect, use = "pairwise")
